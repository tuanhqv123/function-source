    ...
    try:
        ...
        for item in datas:
            ...
            if item[0] > 240 and item[1] > 240 and item[2] > 240:
                ...
                newData.append(item)  # Giữ nguyên pixel
        img.putdata(newData)
        ...
        canvas_width = max(
            img_resized.width,
            signed_by_size[0],
            signature_valid_size[0],
            title_size[0],
            date_size[0]
        ) + 40
        canvas_height = (img_resized.height
                         + signed_by_size[1]
                         + signature_valid_size[1]
                         + title_size[1]
                         + date_size[1]
                         + 60)
        ...
        draw.text((text_left_margin, full_name_y), full_name, fill=(0, 0, 0), font=font)

        # Vẽ các dòng văn bản
        text_color_red = (255, 0, 0)  # Màu đỏ cho các thông tin khác
        signature_valid_y = full_name_y + 20
...
def add_signature():
    ...
    try:
        ...
        placeholder = request.form.get('placeholder', 'Signature')  # Placeholder mặc định
        ...
        for page_num in range(len(pdf_document)):
            ...
            # Sao chép nội dung trang gốc sang trang mới trong output_pdf
            new_page = output_pdf.new_page(width=page.rect.width, height=page.rect.height)
            new_page.show_pdf_page(page.rect, pdf_document, pno=page_num)

            # Nếu trang này có placeholder và chưa ký lần nào:
            if text_instances and not already_signed:
                logging.info(f"Tìm thấy placeholder trên trang {page_num+1}, bắt đầu chèn chữ ký.")
                
                # Bước hợp nhất các bounding box để xóa sạch placeholder:
                union_rect = text_instances[0]
                for r in text_instances[1:]:
                    union_rect |= r

                # Xóa placeholder (vẽ hình chữ nhật trắng phủ lên)
                new_page.draw_rect(union_rect, color=(1, 1, 1), fill=(1, 1, 1))

                # Mở ảnh chữ ký đã xử lý
                ...
                # Đặt chữ ký lên chính vùng placeholder (union_rect)
                signature_rect = fitz.Rect(
                    union_rect.x0,
                    union_rect.y0,
                    union_rect.x0 + signature_width,
                    union_rect.y0 + signature_height
                )

                # Nếu chữ ký vượt qua biên trang, điều chỉnh
                ...
                logging.info("Chữ ký đã được chèn thành công.")

                already_signed = True  # Đánh dấu đã ký
                # break  # tùy bạn muốn dừng loop luôn hay vẫn muốn lướt qua các trang sau.
                # Ở đây chỉ ký 1 placeholder cho toàn bộ file, nên break cũng được.

            if already_signed:
                logging.info("Đã ký xong, không chèn ở trang tiếp theo.")
                # break  # Nếu muốn dừng hẳn, bỏ comment dòng này
